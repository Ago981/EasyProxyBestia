...
class DLStreamsExtractor:
    ...
    async def _is_stream_alive(self, session, stream_url: str, headers: dict) -> bool:
        """Download the manifest and check it is a real HLS playlist.

        A candidate can yield a well-formed URL that is actually dead
        (expired token, CDN blocking the server IP/geo). Returning it
        blindly would hide the working players later in the list,
        so every URL is validated before being accepted.
        """
        try:
            async with session.get(stream_url, headers=headers, timeout=10) as resp:
                ...
                alive = "#EXTM3U" in body[:8000] or "#EXT-X-" in body[:8000]
                if not alive:
